...
func init()  {
	...
	Config = ConfigList{
		Port : cfg.Section("web").Key("port").MustInt(),//なければ0
		DbName : cfg.Section("db").Key("name").MustString("example.sql"),//なければ初期値が入る。
		SQLDriver: cfg.Section("db").Key("driver").String(),//なければ空
	}
}
...
func talibPrac()  {
	...
	// rsi2 := talib.Rsi(spy.Close, 2)
	// fmt.Println(rsi2)
	// mva := talib.Ema(spy.Close, 14)//closeしたときの値を、過去１４日間のデータで計算moving average
	// fmt.Println(mva)
}
